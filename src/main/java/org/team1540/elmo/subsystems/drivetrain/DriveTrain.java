...
public class DriveTrain extends SubsystemBase {
    ...
    private void rampingPeriodic() {
        if(driveRampingMode.equals(DriveRampingMode.SPEED_SPIN)) setSpeedSpinRaw(speedRateLimiter.calculate(speedRampedSetpoint),spinRateLimiter.calculate(spinRampedSetpoint));
    ...
    private static Vector2d preprocessSpeedSpin(double speedPercent, double spinPercent) {
        ...
        double maxScalarPercent = Math.abs(speedPercent)+Math.abs(spinPercent/2d);
        ...
        maxScalarPercent = Math.abs(speedPercent)+Math.abs(spinPercent/2d);
    ...
    private void setSpeedSpinRaw(double speedPercent, double spinPercent) {
        ...
        leftPercent += spinPercent/2d;
        rightPercent -= spinPercent/2d;
        setPercentRaw(leftPercent,rightPercent);
    }

    public void setSpeedSpin(double speedPercent, double spinPercent) {
    ...
    private final double defaultSpinAccLimit = 9;
    private final double defaultSpinDecLimit = 9;
    ...
    private SignedSlewRateLimiter speedRateLimiter = new SignedSlewRateLimiter(
    ...
    private SignedSlewRateLimiter spinRateLimiter = new SignedSlewRateLimiter(
